...
func load_level(path: String) -> bool:
	...
	# CRITICAL: Load polygon BEFORE updating wall bounds
	...
	# CRITICAL: Create top edge holds AFTER wall bounds are set
...
# =============================================================================
# ENVIRONMENT
# Dynamically matches the saved environment name against EnvironmentConfig.
# Adding a new EnvironmentType to EnvironmentConfig automatically works here.
# =============================================================================
func set_environment_from_string(env_name: String):
	...
	# Iterate all registered environment types and match by name
	for env_type in env_config.get_all_environment_types():
		if env_config.get_environment_name(env_type).to_lower() == env_name.to_lower():
			...
			print("Level environment set to: " + env_config.get_environment_name(env_type).to_upper())
			matched = true
			break

	if not matched:
		print("WARNING: Unknown environment '" + env_name + "', defaulting to first registered environment")
...
func get_top_holds() -> Array[Node2D]:
	...
	# Also include dynamically created top edge holds from wall
	if dynamic_wall:
		for child in dynamic_wall.get_children():
			if child.has_meta("is_top_edge_hold"):
				tops.append(child)
