...
## Convert a level path — if it's an old .tscn path, convert to .json.
## E.g. "res://scenes/levels/tutorial/tutorial_01.tscn" → "res://data/levels/tutorial/tutorial_01.json"
## .json paths are returned as-is.
...
func load_save_data(data: Dictionary) -> void:
	# Migrate old .tscn save keys to new .json keys
	var raw_completed = data.get("completed_levels", {})
	completed_levels = {}
	for level_path in raw_completed:
		var new_path = level_path.replace(".tscn", ".json").replace("res://scenes/levels/", "res://data/levels/")
		if new_path == level_path:
			new_path = level_path  # already .json or other format
