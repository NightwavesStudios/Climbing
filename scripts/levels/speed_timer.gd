...
func update_display():
	...
	# Colour feedback for urgency
	if time_remaining <= 5.0:
		timer_label.add_theme_color_override("font_color", Color(1.0, 0.15, 0.15))  # red
	elif time_remaining <= 10.0:
		timer_label.add_theme_color_override("font_color", Color(1.0, 0.70, 0.10))  # amber
	else:
		timer_label.add_theme_color_override("font_color", Color.WHITE)
